...
final class CalendarStore: ObservableObject, FirestoreCacheStore {
    ...
    func save(schedule: DoseSchedule) {
        let existing = loadAll(forMedicationId: schedule.medicationId)
        let entries = Self.entriesSurviving(existing, regeneratingOn: schedule.startDate) + schedule.buildEntries()
        persist(entries, id: schedule.medicationId)
        startListening(for: schedule.medicationId)
    }

    // Which of a medication's existing entries survive a schedule
    // regeneration (medication added or edited), before the fresh batch
    // from `schedule.buildEntries()` is appended. Pure and separate from
    // the Firestore read/write above so it's testable without live
    // Firebase - see CalendarStoreSaveScheduleTests.
    //
    // - `.upcoming` never survives - always superseded by the fresh
    //   schedule.
    // - `.taken`/`.missed` always survive - real history, never touched by
    //   an unrelated later edit.
    // - `.skipped` survives unless it falls on `day` (the new schedule's
    //   own start day). `.skipped` is only ever generated by
    //   DoseSchedule.buildEntries() as a mid-day-add seed, on the
    //   schedule's own start day (see .claude/rules/dose-tracking.md's
    //   Mid-day medication add section) - editing the medication again
    //   later the same day (e.g. changing the dose time) used to leave the
    //   old seed behind as stale clutter instead of replacing it with a
    //   fresh one for the current schedule. Scoped to `day` only: a
    //   `.skipped` entry from a past day is settled history from that
    //   day's own mid-day add and must never be touched by a later,
    //   unrelated edit.
    static func entriesSurviving(_ entries: [DoseEntry], regeneratingOn day: Date) -> [DoseEntry] {
        let cal = Calendar.current
        return entries.filter { entry in
            ...
            case .upcoming:
                return false
            case .skipped:
                return !cal.isDate(entry.scheduledDate, inSameDayAs: day)
            case .taken, .missed:
                return true
            }
        }
