...
class MedicationAnalyzer {
    ...
    func detectMedicationFromLabel(image: UIImage, completion: @escaping (DetectedMedicationInfo) -> Void) {
        guard let cgImage = image.cgImage else {
            completion(DetectedMedicationInfo(brandName: "", genericName: "", schedule: "", dosage: "", scheduleTime: Date(), allDetectedText: []))
            return
        }
        ...
        recognizeText(in: cgImage) { texts in
            ...
            Task {
                let info = await self.extractWithGemini(ocrTexts: texts)
                await MainActor.run { completion(info) }
            }
        }
    }

    // MARK: - Gemini extraction

    private func extractWithGemini(ocrTexts: [String]) async -> DetectedMedicationInfo {
        ...
        let urlString = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=\(Config.geminiAPIKey)"
        guard let url = URL(string: urlString),
              let body = try? JSONSerialization.data(withJSONObject: requestBody) else {
            print("❌ Gemini: failed to build request")
            return emptyInfo(allDetectedText: ocrTexts)
        }
        ...
        do {
            ...
            guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
                  ...
                  let extracted = try? JSONSerialization.jsonObject(with: textData) as? [String: String] else {
                print("❌ Gemini: failed to parse response — raw: \(String(data: data, encoding: .utf8) ?? "nil")")
                return emptyInfo(allDetectedText: ocrTexts)
            }

            let brandName = extracted["brandName"] ?? ""
            let genericName = extracted["genericName"] ?? ""
            let dosage = extracted["dosage"] ?? ""
            let schedule = extracted["schedule"] ?? ""
            ...
            return DetectedMedicationInfo(
                ...
                schedule: schedule,
                dosage: dosage,
                scheduleTime: scheduleTimeFromText(schedule),
    ...
    private func emptyInfo(allDetectedText: [String] = []) -> DetectedMedicationInfo {
        DetectedMedicationInfo(brandName: "", genericName: "", schedule: "", dosage: "", scheduleTime: Date(), allDetectedText: allDetectedText)
    }

    private func scheduleTimeFromText(_ schedule: String) -> Date {
    ...
    private func recognizeText(in image: CGImage, completion: @escaping ([String]) -> Void) {
        let request = VNRecognizeTextRequest { request, error in
            ...
            // Sort top-to-bottom by bounding box (higher y = higher on screen in Vision coords)
            let sorted = observations.sorted { $0.boundingBox.origin.y > $1.boundingBox.origin.y }
            // Take only the best candidate per line, preserve spatial order (no alphabetical sort)
            let texts = sorted.compactMap { observation -> String? in
